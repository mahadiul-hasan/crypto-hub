  ...
  return (
    <div
      className={`
        ...
        border-l-4 ${toastStyles[toast.type].border}
        rounded-r-lg shadow-lg overflow-hidden
        transform transition-all duration-300 ease-in-out
        hover:shadow-xl pointer-events-auto
        animate-slideIn
      `}
    ...
    >
      <div className="flex items-start p-4">
        <div className="shrink-0">{toastStyles[toast.type].icon}</div>
        <div className="ml-3 flex-1">
          <p className="text-sm text-gray-700">{toast.message}</p>
        </div>
        <button
          onClick={() => hideToast(toast.id)}
          className="ml-4 shrink-0 text-gray-400 hover:text-gray-600 transition-colors focus:outline-none"
      ...
      {toast.duration && toast.duration > 0 && (
        <div className="h-1 bg-gray-200" style={{ width: "100%" }}>
          <div
            className={`h-full ${
              toast.type === "success"
                ? "bg-green-500"
